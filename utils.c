//---------------------------------------------------------------------------
// Различные утилиты, коорые используются модулями
#include <pthread.h>
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
...
u16 test_crc16(u8 * mes, int size)
{
	...
	if ((size == sizeof(DEV_STATUS_STRUCT) || size == sizeof(ADS1282_PACK_STRUCT)) && len != size) {
		...
		if (len == size) {
		    ...
		    j = 1;
		    if (i > 1)
			printf("WARN: move header from pos %d: len %d, size %d!\n", i, len, size);
...
void time_to_str(char *str)
{
	...
	sprintf(str, "G %02d-%02d-%04d %02d:%02d:%02d.%03d ", t.day, t.mon, t.year, t.hour, t.min, t.sec,
